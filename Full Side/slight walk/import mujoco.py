...
lift_R = {
    "left_hip_yaw": 0.0, "left_hip_roll": -angle, "left_hip_pitch": -0.15, "left_knee_pitch": 0.3, "left_ankle_pitch": -0.15, "left_ankle_roll": angle,
    "right_hip_yaw": 0.0, "right_hip_roll": -angle, "right_hip_pitch": -0.35, "right_knee_pitch": 0.5, "right_ankle_pitch": -0.15, "right_ankle_roll": angle
}

plant_R = {
    "left_hip_yaw": 0.0, "left_hip_roll": -angle, "left_hip_pitch": -0.15, "left_knee_pitch": 0.3, "left_ankle_pitch": -0.15, "left_ankle_roll": angle,
    "right_hip_yaw": 0.0, "right_hip_roll": -angle, "right_hip_pitch": -0.2, "right_knee_pitch": 0.3, "right_ankle_pitch": -0.15, "right_ankle_roll": angle
}
...
lift_L = {
    "left_hip_yaw": 0.0, "left_hip_roll": angle, "left_hip_pitch": -0.35, "left_knee_pitch": 0.5, "left_ankle_pitch": -0.15, "left_ankle_roll": -angle,
    "right_hip_yaw": 0.0, "right_hip_roll": angle, "right_hip_pitch": -0.15, "right_knee_pitch": 0.3, "right_ankle_pitch": -0.15, "right_ankle_roll": -angle
}

plant_L = {
    "left_hip_yaw": 0.0, "left_hip_roll": angle, "left_hip_pitch": -0.2, "left_knee_pitch": 0.3, "left_ankle_pitch": -0.15, "left_ankle_roll": -angle,
    "right_hip_yaw": 0.0, "right_hip_roll": angle, "right_hip_pitch": -0.15, "right_knee_pitch": 0.3, "right_ankle_pitch": -0.15, "right_ankle_roll": -angle
}

# ==========================================================
# TWO-PHASE SHIFT: go to CENTER first, then to target foot
# This prevents the COM from overshooting by breaking the
# full-width shift into two half-width shifts!
# ==========================================================

center = dict(stand)  # center = standing straight, roll=0
...
# 0: SIT
# 1: STAND
# 2: SHIFT_TO_LEFT (center -> lean_L)
# 3: LIFT_RIGHT
# 4: PLANT_RIGHT
# 5a: SHIFT_TO_CENTER (lean_L -> center)  -- NEW!
# 5b: SHIFT_TO_RIGHT (center -> lean_R)   -- NEW!
# 6: LIFT_LEFT
# 7: PLANT_LEFT
# 8a: SHIFT_TO_CENTER (lean_R -> center)  -- NEW!
# 8b: SHIFT_TO_LEFT (center -> lean_L)    -- NEW!
# Then cycle 3->4->5a->5b->6->7->8a->8b->3
...
MIN_DURATIONS = {
    0: 0.5,
    1: 1.5,
    2: 2.0,    # initial shift to left (half width)
    3: 0.4,    # lift right
    4: 0.3,    # plant right
    50: 1.5,   # shift lean_L -> center (5a)
    51: 1.5,   # shift center -> lean_R (5b)
    6: 0.4,    # lift left
    7: 0.3,    # plant left
    80: 1.5,   # shift lean_R -> center (8a)
    81: 1.5,   # shift center -> lean_L (8b)
    99: 999,   # fallen
}
...
# State -> (source_pose, target_pose)
def get_poses_for_state(st):
    if st == 0: return sit, sit
    if st == 1: return sit, stand
    if st == 2: return stand, lean_L
    if st == 3: return lean_L, lift_R
    if st == 4: return lift_R, plant_R
    if st == 50: return lean_L, center    # lean_L -> center
    if st == 51: return center, lean_R    # center -> lean_R
    if st == 6: return lean_R, lift_L
    if st == 7: return lift_L, plant_L
    if st == 80: return lean_R, center    # lean_R -> center
    if st == 81: return center, lean_L    # center -> lean_L
    return stand, stand
...
KP = {"yaw": 600, "roll": 1200, "hip": 1200, "knee": 1500, "ankle_pitch": 1000, "ankle_roll": 800}
KD = {"yaw": 60, "roll": 200, "hip": 150, "knee": 180, "ankle_pitch": 150, "ankle_roll": 120}
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    while viewer.is_running():
        ...
        if state == 2 and can_transition:
            if not check_com_over_foot('L', 0.04):
                can_transition = False
                if state_elapsed > min_dur + 3.0:
                    print(f"  WARNING: COM timeout state {state}")
                    can_transition = True
        elif state == 51 and can_transition:
            if not check_com_over_foot('R', 0.04):
                can_transition = False
                if state_elapsed > min_dur + 3.0:
                    can_transition = True
