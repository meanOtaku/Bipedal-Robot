...
lift_L_turn_L = {
    "left_hip_yaw": 0.4, "left_hip_roll": angle, "left_hip_pitch": -0.15, "left_knee_pitch": 0.60, "left_ankle_pitch": -0.15, "left_ankle_roll": -angle,
    "right_hip_yaw": 0.0, "right_hip_roll": angle, "right_hip_pitch": -0.15, "right_knee_pitch": 0.30, "right_ankle_pitch": -0.15, "right_ankle_roll": -angle
}

plant_L_turn_L = {
    "left_hip_yaw": 0.4, "left_hip_roll": angle, "left_hip_pitch": -0.15, "left_knee_pitch": 0.30, "left_ankle_pitch": -0.15, "left_ankle_roll": -angle,
    "right_hip_yaw": 0.0, "right_hip_roll": angle, "right_hip_pitch": -0.15, "right_knee_pitch": 0.30, "right_ankle_pitch": -0.15, "right_ankle_roll": -angle
}

center_L_turn_L = {
    "left_hip_yaw": 0.2, "left_hip_roll": 0.0, "left_hip_pitch": -0.15, "left_knee_pitch": 0.30, "left_ankle_pitch": -0.15, "left_ankle_roll": 0.0,
    "right_hip_yaw": -0.2, "right_hip_roll": 0.0, "right_hip_pitch": -0.15, "right_knee_pitch": 0.30, "right_ankle_pitch": -0.15, "right_ankle_roll": 0.0
}

lean_L_turn_L = {
    "left_hip_yaw": 0.0, "left_hip_roll": -angle, "left_hip_pitch": -0.15, "left_knee_pitch": 0.30, "left_ankle_pitch": -0.15, "left_ankle_roll": angle,
    "right_hip_yaw": -0.4, "right_hip_roll": -angle, "right_hip_pitch": -0.15, "right_knee_pitch": 0.30, "right_ankle_pitch": -0.15, "right_ankle_roll": angle
}
...
lift_R_turn_R = {
    "left_hip_yaw": 0.0, "left_hip_roll": -angle, "left_hip_pitch": -0.15, "left_knee_pitch": 0.30, "left_ankle_pitch": -0.15, "left_ankle_roll": angle,
    "right_hip_yaw": -0.4, "right_hip_roll": -angle, "right_hip_pitch": -0.15, "right_knee_pitch": 0.60, "right_ankle_pitch": -0.15, "right_ankle_roll": angle
}
    ...
    while viewer.is_running():
        ...
        if state == 10:
            if key_up:
                ...
                state = 2
                state_start = elapsed
                print(f"  t={elapsed:.1f}s  IDLE->WALK_FORWARD")
            elif key_down:
                walk_mode = "backward"
                state = 2
                state_start = elapsed
                print(f"  t={elapsed:.1f}s  IDLE->WALK_BACKWARD")
            elif key_left:
                walk_mode = "turn_left"
                state = 2
                state_start = elapsed
                target_yaw = unwrapped_yaw + (np.pi / 2) - 0.2
                print(f"  t={elapsed:.1f}s  IDLE->TURN_LEFT")
            elif key_right:
                walk_mode = "turn_right"
                state = 2
                state_start = elapsed
                target_yaw = unwrapped_yaw - (np.pi / 2) + 0.2
                print(f"  t={elapsed:.1f}s  IDLE->TURN_RIGHT")
                ...
                if state_elapsed > 1.0:
                    ...
                    state = 2
                    state_start = elapsed
                    print(f"  t={elapsed:.1f}s  IDLE->AUTO_FORWARD (no keyboard)")
        ...
        if can_transition and state != 99 and state != 10:
            old_state = state
            if state == 0: state = 1
            elif state == 1: state = 10  # Go to IDLE after standing up
                ...
                if key_up:
                    ...
                    if walk_mode != "turn_left": target_yaw = unwrapped_yaw + (np.pi / 2) - 0.2
                    walk_mode = "turn_left"
                    state = 6
                elif key_right:
                    if walk_mode != "turn_right": target_yaw = unwrapped_yaw - (np.pi / 2) + 0.2
                    walk_mode = "turn_right"
                    state = 6
                else:
                    if walk_mode == "turn_left" and unwrapped_yaw < target_yaw:
                        state = 6
                    elif walk_mode == "turn_right" and unwrapped_yaw > target_yaw:
                        state = 6
                    else:
                        state = 90  # Settle and stop
                        print(f"  t={elapsed:.1f}s  SETTLING (from right lean)")
                ...
                if key_up:
                    ...
                    if walk_mode == "turn_left" and unwrapped_yaw < target_yaw:
                        ...
                        state = 91  # Settle and stop
                        print(f"  t={elapsed:.1f}s  SETTLING (from left lean)")
            elif state == 90:
                state = 10
                walk_mode = "idle"
                step_count = 0
            ...
            state_start = elapsed
            if old_state != state and state not in [10, 90, 91]:
                com_y = get_com_y()
        ...
        for actuator_id, joint_name in enumerate(joint_names):
            ...
            if walk_mode in ["forward", "backward"]:
                if joint_name in ["left_hip_yaw", "right_hip_yaw"]:
                    bias = 0.0
                    if key_left: bias = 0.25
                    elif key_right: bias = -0.25
                    
                    # Swing Collision Avoidance: Prevent swinging into the other leg
                    if walk_mode == "forward":
                        if joint_name == "right_hip_yaw" and bias > 0 and state in [3, 4]:
                            bias = 0.0  # Right leg swinging inward -> straighten
                        elif joint_name == "left_hip_yaw" and bias < 0 and state in [6, 7]:
                            bias = 0.0  # Left leg swinging inward -> straighten
                    elif walk_mode == "backward":
                        if joint_name == "right_hip_yaw" and bias < 0 and state in [3, 4]:
                            bias = 0.0  # Right leg swinging outward (which swings inward backwards) -> straighten
                        elif joint_name == "left_hip_yaw" and bias > 0 and state in [6, 7]:
                            bias = 0.0  # Left leg swinging outward (which swings inward backwards) -> straighten
