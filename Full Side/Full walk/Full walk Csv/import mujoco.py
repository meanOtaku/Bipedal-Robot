...
# Right goes from -0.5 to -0.15. Left trails from -0.15 to +0.20. 
center_R = {
...
# Left goes from -0.5 to -0.15. Right trails from -0.15 to +0.20.
center_L = {
...
def is_fallen():
    ...
    # Fall if tilted more than 45 degrees (0.8 rads) in either direction
...
def get_gains(name):
    ...
    if "ankle_pitch" in name:
        # Dynamic Compliance: Relax the trailing ankle so it naturally lifts the heel without sliding or catapulting
...
telemetry_data = []
telemetry_headers = ["time", "state", "torso_x", "torso_y", "torso_z", "imu_roll", "imu_pitch", "imu_yaw"]
for jn in joint_names:
...
with mujoco.viewer.launch_passive(model, data) as viewer:
    ...
    while viewer.is_running():
        ...
        if elapsed - last_print > 1.0 and state >= 2 and state != 99:
            ...
            print(f"    [diag] t={elapsed:.1f}s st={state} roll={roll:.3f} pitch={pitch:.3f}")
            last_print = elapsed

        # --- TELEMETRY COLLECTION (100Hz) ---
        if step_counter % 10 == 0:
            ...
            for actuator_id, joint_name in enumerate(joint_names):
                ...
                # Actual applied torque is ctrl * gear
                gear = model.actuator_gear[actuator_id, 0]
